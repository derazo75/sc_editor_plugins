...
(function () {
  ...
  function inyectarEstilos() {
    ...
    GM_addStyle(`
            ...
            .cm-comment {font-style: italic; backdrop-filter: brightness(0.3); filter: brightness(1.5) sepia(1) hue-rotate(140deg) saturate(10); mix-blend-mode: screen; }
  ...
  function ejecutarFormateadorServer(cm) {
    ...
    fetch("http://localhost:5005/format", {
      method: "POST",
  ...
  function ejecutarIA(cm, lineText, cursor) {
    ...
    if (seleccion && seleccion.trim().length > 0) {
      // ESCENARIO 2: Selección (de la 50 a la 80)
      const lineasSeleccionadas = seleccion.split("\n");
      instruccion = lineasSeleccionadas[0].trim(); // Línea 50 (Instrucción)

      // Enviamos el resto del código seleccionado como contexto (líneas 51 a 80)
      contextoIA = lineasSeleccionadas.slice(1).join("\n");
    } else {
      // ESCENARIO 1: Solo la línea 50
      instruccion = lineText.trim();
      contextoIA = ""; // No enviamos el resto del archivo, solo la orden
    }
    ...
    if (instruccion.length > 3) {
      ...
      panelLint.innerHTML = "⏳ <strong>IA:</strong> Procesando...";
      ...
      fetch("http://localhost:5005/ask", {
        ...
        .then((res) => res.json())
        .then((data) => {
          // Formateamos la respuesta para que mantenga tu comentario original arriba
          const respuestaFinal = instruccion + "\n" + data.code;

          if (seleccion) {
            // Reemplaza todo el bloque (50 a 80) con: Comentario + Código Nuevo
            cm.replaceSelection(respuestaFinal);
          } else {
            // Reemplaza la línea actual con: Comentario + Código Nuevo
